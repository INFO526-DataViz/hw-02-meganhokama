...
summary_table <- uk_contributions %>%
  group_by(party, year) %>% 
  summarize(total_contribution = count(uk_contributions$amount))
...
ggplot(summary_table,aes(x = year, 
  ...
  scale_color_manual(values = c(Democrat = "blue", Republican = "red")) + # Giving colors manually
  labs(x = "Year",
       y = "Total amount",
  ...
  theme_minimal() +
  theme(
    legend.position = c(0.9,0.17), # Changing legend position to replicate the required plot
    axis.title = element_text(hjust = 0,vjust=0)
  ) +
    scale_y_continuous(labels = dollar_format(scale = 1e-06, suffix = "M"))
```
```{r}
...
summary_table_france <- france_contributions %>%
  group_by(party, year) %>% 
  summarize(total_contribution_france = count(france_contributions$amount))
```

``` {r Plot the data}

ggplot(summary_table_france,aes(x = year, 
                         y = total_contribution_france, 
                         group = party,
                         color = party)) +
  geom_line(linewidth = 0.95) +
  scale_color_manual(values = c(Democrat = "blue", Republican = "red")) + # Giving colors manually
...
ggplot(aes(x=size,y=distance))+
  geom_segment(aes(xend=size,yend=0),color="blue",linewidth=1)+
  geom_text(aes(label ="✈️"), size = c(16,12,8,4), show.legend = FALSE)+    #flight emoji label found here:     https://symbl.cc/en/2708/
  labs(x="Plane size",
       y="Distance",
       title="Number of flights out of RDU in 2020",
...
#Scatterplot to compare flight delays by manufacturer and plane size
